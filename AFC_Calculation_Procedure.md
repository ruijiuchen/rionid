...
> **注意**：该文件在每次点击 Find Peaks 时被覆盖。`area` 列是原始积分面积，如需扣除本底的净面积需从 `afc_harmonics.csv` 或代码内部数据结构中获取。
...
### 11.3 `afc_norm_area.csv` — 归一化面积与共振拟合

点击 **Plot Norm Area** → **Fit & Plot** 时生成。
...
| `voltage_V` | 电压值 | V |
| `freq_MHz` | 峰频率 | MHz |
| `norm_area` | 归一化面积（每个投影内以参考峰归一化） | — |
| `fitted_curve` | 共振拟合曲线在对应频率处的值 | — |
| `residual` | 归一化面积与拟合值的差 | — |

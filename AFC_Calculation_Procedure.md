...
MC (500 iter): Q = 10415 ⨦ 168    f_sys = 308.125000 ⨦ 5.10e-02 MHz
```
...
```
raw_area = peak_area - bg_area                # 净面积（每帧平均）
raw_area_err = area_err                         # 面积标准误
```

### 10.2 曲线（含误差棒）
- **蓝色曲线** — 原始净面积 vs 时间（`errorbar` 显示标准误）
- **粉色曲线** — AFC 修正后面积 vs 时间：`corrected = raw / H(freq)`，使用与 Plot Norm Area 一致的 H(f) 模型，**不归一化**
- **绿色曲线** — 加权指数衰减拟合：`A(t) = A₀·exp(-λ × t)`，使用 `scipy.optimize.curve_fit`

### 10.3 参数控制
Q、f_sys 参数可手动调节（自动填入 Plot Norm Area 的拟合结果），点击 **Update** 更新图形。A₀ 参数在 H(f) 模型中已消去。
...
点击 **Plot Norm Area** 时自动生成。每行包含面积比数据及对应的分子、分母峰面积及其误差。
...
| `area1` | 分子峰面积 | arb. unit |
| `area1_err` | 分子峰面积误差 | arb. unit |
| `area_ref` | 分母（参考）峰面积 | arb. unit |
| `area_ref_err` | 分母峰面积误差 | arb. unit |
